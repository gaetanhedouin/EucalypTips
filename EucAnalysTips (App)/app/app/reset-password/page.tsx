  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(payload?.message ?? 'Reinitialisation impossible.');
      }
    ...
    <main className="grid min-h-screen place-items-center bg-gradient-to-b from-[#080d15] to-[#0a1120] p-6 text-[#f2fbfa]">
      <section className="w-full max-w-[560px] rounded-[18px] border border-white/10 bg-[#0e1624] p-6 shadow-[0_14px_40px_rgba(0,0,0,0.4)]">
        ...
        <form onSubmit={onSubmit}>
          <label className="mb-1 mt-4 block text-sm text-[#c5d2d9]" htmlFor="password">
          ...
          <label className="mb-1 mt-3 block text-sm text-[#c5d2d9]" htmlFor="confirm">
          ...
          <button
            className="mt-4 inline-flex items-center justify-center rounded-xl bg-[#7de8d3] px-4 py-2.5 font-bold text-[#0b2622] transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-60"
            type="submit"
            disabled={loading}
          >
        ...
        <Link
          className="mt-4 inline-flex items-center justify-center rounded-full border border-white/10 bg-white/5 px-4 py-2 text-[#c5d2d9] transition-colors hover:bg-white/10"
          href="/"
        >
