  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(await parseApiError(response, 'Impossible de verifier cet email.'));
        }
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-gradient-to-b from-[#080d15] to-[#0a1120] p-6 text-[#f2fbfa]">
      <section className="w-full max-w-[560px] rounded-[18px] border border-white/10 bg-[#0e1624] p-6 shadow-[0_14px_40px_rgba(0,0,0,0.4)]">
        ...
        <p className="mb-5 mt-2 text-sm text-[#c5d2d9]">{message}</p>
        {!loading ? (
          <Link className="inline-flex items-center justify-center rounded-xl bg-[#7de8d3] px-4 py-2.5 font-bold text-[#0b2622] transition-opacity hover:opacity-90" href="/auth">
